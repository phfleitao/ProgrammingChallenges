...
namespace HackerRank
{
    ...
    public class Problems
    {
        ...
        public static void plusMinus(int[] arr)
        {
            ...
            Console.WriteLine(((double)countPlus / (double)arr.Length).ToString("0.000000", CultureInfo.InvariantCulture));
            Console.WriteLine(((double)countMinus / (double)arr.Length).ToString("0.000000", CultureInfo.InvariantCulture));
            Console.WriteLine(((double)countZero / (double)arr.Length).ToString("0.000000", CultureInfo.InvariantCulture));
        }
        ...
        {
            candles.Sort();
            int tallest = candles[candles.Count - 1];
            int count = 0;
            for (int i = candles.Count - 1; i >= 0; i--)
            {
                if (candles[i] != tallest) break;
                count++;
            }
            return count;
        }
            ...
            foreach (var composicao in composicoes)
            {
                ...
                else
                {
                    ...
                    for (int i = 0; i < (composicao.Count - 1); i++)
                    {
                        ...
                        expressoes.Add(expressao, Convert.ToInt64(new DataTable().Compute(expressao, null)));
